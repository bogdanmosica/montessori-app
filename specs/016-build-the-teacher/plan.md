...
# Implementation Plan: Teacher Management Page

**Branch**: `016-build-the-teacher` | **Date**: October 3, 2025 | **Spec**: [spec.md](./spec.md)
**Input**: Feature specification from `/specs/016-build-the-teacher/spec.md`
...
## Summary
Build a Teacher Management page (`/admin/teachers`) for school administrators to manage teacher accounts and student assignments. The page enables CRUD operations on teachers with soft deletion, multi-teacher student assignments (co-teaching), and silent account creation. Technical approach: Next.js page with server components, shadcn/ui components, multi-tenant RBAC enforcement, and Drizzle ORM for database operations.

## Technical Context
**Language/Version**: TypeScript with Next.js 15 (App Router), React 19  
**Primary Dependencies**: Next.js, shadcn/ui, Tailwind CSS, Drizzle ORM, Auth.js  
**Storage**: PostgreSQL with multi-tenant data isolation  
**Testing**: Jest with React Testing Library (note: skip test tasks per user request)  
**Target Platform**: Web application (responsive design)  
**Project Type**: web - Next.js application structure  
**Performance Goals**: Standard web app performance (<200ms page loads)  
**Constraints**: Multi-tenant security, RBAC enforcement, constitutional compliance  
**Scale/Scope**: Admin module page with CRUD operations, 3-5 components, teacher-student relationship management
**User Context**: Page is already implemented so implementation approach is known
...
**Structure Decision**: Option 2 - Next.js application structure (web app detected)
...
**Task Generation Strategy** (Modified per user request):
- Load `.specify/templates/tasks-template.md` as base
- Generate tasks from Phase 1 design docs (contracts, data model, quickstart)
- Skip test tasks as requested ("skip test tasks, the page is already implemented")
- Focus on implementation tasks based on existing page structure
- Each entity → model creation task [P]
- Each API endpoint → route implementation task [P]
- Each component → component implementation task [P]
- Database migration tasks for new schema
...
**User Context Applied**: Page is already implemented, so tasks will focus on ensuring compliance with specification and constitutional requirements rather than full TDD approach.
